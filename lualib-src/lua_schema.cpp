...
static void do_verify(lua_State* L, const std::string& proto_name, int index, std::vector<std::string>& trace);

static void check_field_type(lua_State* L, int index, const std::string& proto_name, const std::string& field_name, const std::string& name, const std::string& type, std::vector<std::string>& trace)
{
	...
	else if (0 == ok)
	{
		...
		if (name.empty())
		{
			throw std::logic_error{moon::format("'%s.%s' %s expected, got %s, value '%s'. trace: %s", proto_name.data(), field_name.data(), type.data(), luaL_typename(L, index), value.data(), trace_to_string(trace).data())};
		}
		else
		{
			throw std::logic_error{moon::format("'%s.%s.%s' %s expected, got %s. trace: %s", proto_name.data(), field_name.data(), name.data(), type.data(), luaL_typename(L, index), trace_to_string(trace).data())};
		}
	}
}

static void verify_field(lua_State* L, int index, const std::string& proto_name, const std::unordered_map<std::string, proto_field>& proto, const char* field_name, std::vector<std::string>& trace)
{
	...
	{
		throw std::logic_error{moon::format("Attemp to index undefined field: '%s.%s'. trace: %s", proto_name.data(), field_name, trace_to_string(trace).data())};
	}
	else
	{
		...
		case compose_type::T_ARRAY:
		{
			...
			{
				throw std::logic_error{moon::format("'%s.%s' table expected, got %s. trace: %s", proto_name.data(), field_name, luaL_typename(L, index), trace_to_string(trace).data())};
			}

			auto size = array_size(L, index);
			if (size == std::numeric_limits<size_t>::max())
			{
				throw std::logic_error{moon::format("'%s.%s' not meet lua array requirements. trace: %s", proto_name.data(), field_name, trace_to_string(trace).data())};
			}
			...
			for (size_t i = 1; i <= size; i++)
			{
				...
				check_field_type(L, -1, proto_name, field_name, std::to_string(i), field.value_type, trace);
				lua_pop(L, 1);
				trace.pop_back();
			}
			break;
		...
		case compose_type::T_OBJECT:
		{
			...
			while (lua_next(L, index) != 0)
			{
				...
				check_field_type(L, -1, proto_name, field_name, key_value, field.value_type, trace);
				trace.pop_back();
				lua_pop(L, 1);
			}
			break;
...
static void do_verify(lua_State* L, const std::string& proto_name, int index, std::vector<std::string>& trace)
{
	...
	{
		throw std::logic_error{moon::format("'%s' table expected, got %s. trace: %s", proto_name.data(), luaL_typename(L, index), trace_to_string(trace).data())};
	}

	auto iter = schema_define.find(proto_name);
	if (iter == schema_define.end())
	{
		throw std::logic_error{moon::format("Attemp using undefined proto: %s. trace: %s", proto_name.data(), trace_to_string(trace).data())};
	}

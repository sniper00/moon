...
static int lnext(lua_State* L) {
    ...
    if (type == 0) {
        ...
        int64_t v = (u.channel << UID_CHANNEL_LEFT_SHIFT) | (u.serverid << UID_SERVERID_LEFT_SHIFT)
            | (u.boottimes << UID_BOOTTIMES_LEFT_SHIFT) | (sequence);
        lua_pushinteger(L, v);
        return 1;
    } else {
        ...
        int64_t v = (int64_t { 1 } << FLAG_LEFT_SHIFT) | (u.serverid << SERVERID_LEFT_SHIFT)
            | (u.boottimes << BOOTTIMES_LEFT_SHIFT) | (type << TYPE_LEFT_SHIFT) | (sequence);
        lua_pushinteger(L, v);
        return 1;
    }
}

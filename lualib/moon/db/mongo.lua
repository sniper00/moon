--This file is modified version from https://github.com/cloudwu/skynet/blob/master/lualib/skynet/db/mongo.lua
...
function auth_method:auth_mongodb_cr(user,password)
	local password = md5(string.format("%s:mongo:%s",user,password))
	local result= self:runCommand "getnonce"
	if result.ok ~=1 then
		return false
	end

	local key =	md5(string.format("%s%s%s",result.nonce,user,password))
...
function auth_method:auth_scram_sha1(username,password)
	...
	local pbkdf2_key = md5(string.format("%s:mongo:%s",username,password))

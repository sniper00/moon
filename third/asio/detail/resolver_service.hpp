...
public:
  ...
  results_type resolve(implementation_type&, const query_type& query_,
      asio::error_code& ec)
  {
    asio::detail::addrinfo_type* address_info = 0;

    socket_ops::getaddrinfo(query_.host_name().c_str(),
        query_.service_name().c_str(), query_.hints(), &address_info, ec);
    auto_addrinfo auto_address_info(address_info);

    return ec ? results_type() : results_type::create(
        address_info, query_.host_name(), query_.service_name());
  }

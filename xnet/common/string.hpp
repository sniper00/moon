...
namespace moon
{
	...
	*/
	...
	inline std::unordered_map<std::string, std::string> parse_key_value_string(const std::string& v)
	{
		...
		auto vec = moon::split<std::string>(v, ";");
		for (auto& it : vec)
		{
			moon::trimleft(it);
			moon::trimright(it);
			auto pairs = moon::split<std::string>(it, ":");
			if (pairs.size() == 2)
			{
	...
	//! case insensitive
	inline bool iequals(const std::string&str1, const std::string& str2, const std::locale& Loc = std::locale())
	{
